...
def add_test(args):
    ...
    if args.svn:
        ...
        for obj in (chc, txt, skel, prof, gold):
            cmd ='svn add ' + obj
            if os.system(cmd) == 0:
                print('Added {} to SVN. Don\'t forget to commit'.format(name))
            else:
                print('Could not add {} to SVN'.format(name))
    ...
    for obj in (chc, txt, skel, prof, gold):
        if obj is not None:
            ...
            if args.svn:
                cmd = 'svn del ' + obj
                if os.system(cmd) == 0:
                    print('Removed {} from SVN. Don\'t forget to commit'.format(name))
                else:
                    print('Failed to remove {} from SVN'.format(name))
