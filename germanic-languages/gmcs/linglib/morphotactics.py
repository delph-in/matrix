...
def position_class_hierarchy(choices):
  ...
  for i, pc in enumerate(all_position_classes(choices)):
    ...
    create_lexical_rule_types(cur_pc, pc, choices)
...
def create_lexical_rule_types(cur_pc, pc, ch):
  lrt_parents = {}
  for j, lrt in enumerate(pc.get('lrt')):
    ...
    cur_lrt = create_lexical_rule_type(lrt, ch)
...
def create_lexical_rule_type(lrt, ch):
  ...
  for feat in lrt.get('feat'):
    if feat['name'] == 'worest':
      ...
      if feat['value'] == 'obligatory':
        feat['value'] = 'na-or-+'
    if feat.get('head') == 'subj' and ch.get('verb-cluster') == 'yes':
      subj_feat = True
...
def write_i_or_l_rules(ch, irules, lrules, lrt, order):
  ...
  if any(len(lri) > 0 for lri in lrt.lris):
    ...
    if ch.get('verb-cluster') == 'yes' and len(lrt.lris) == 2:
      lri = lrt.lris[1]
