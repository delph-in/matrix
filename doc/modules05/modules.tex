...
\addtocounter{footnote}{1}
\footnotetext{The situation in English is a bit more complicated.  \newcite{Kim00} argues that sentential {\it not} is actually selected by the auxiliary it appears with.}

To do the evaluation, we ran the Perl script once for each language, answering
the multiple-choice questions with single key strokes; fewer than ten 
keystrokes were required for any one grammar, excluding input for spelling.
The script then
automatically generated the language-specific grammar files, as well as the
customized load file which we used to load the new grammar.  We then batch
parsed the small test suite for that language, and repeated the exercise for
the next language.

The coverage of these automatically generated grammars for the seven languages
is shown in Table~\ref{testresults}.
...
It is clear that the resulting grammars are not all perfect.  For Mandarin,
there is as yet no provision in the modules for A-not-A questions.  For
English, the one example involving a negated yes-no question fails to
parse, since we falsely treated the negation particle as a post-modifier,
which also leads to considerable overgeneration. And for Spanish, the
