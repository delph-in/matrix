...
The purpose of {\sc keys} is to provide constraints for semantic selection
of dependents (complements, specifiers, subjects, and modifiers).  The
attributes in {\it keys} include two called {\sc key} and {\sc altkey}
for constraining the semantic sorts of the dependent phrase, and one called
{\sc message} for constraining types of clausal dependents.
...
where a semantic sort is one of the subtypes defined for the distinguished type
{\it semsort}, intended as a small hierarchy of grammatically salient semantic
types such as {\it animate} or {\it time}, but also the locus for types that
distinguish particular prepositions to support PP-selection.  The value of
this feature is often but not always identified with the value of the 
semantic feature of a sign given by the path 
{\sc SYNSEM.LOCAL.CONT.HOOK.INDEX.SORT} since the semantic sort of a sign
...
are only abbreviations for longer path names).  By making the type 
{\it lexkeys} the value of the feature {\sc lkeys}, the Matrix can ensure
that these abbreviatory features do not propagate to phrases, but are only
used (if at all) in simplifying the definitions of lexical entries.
The features {\sc {-}{-}compkey} and {\sc {-}{-}ocompkey} provide
pointers to the main relations of two complements of a lexical
entry.  The relationship
between {\sc {-}{-}compkey}/{\sc {-}{-}ocompkey} and {\sc keys.key} of
the relevant complements will be established in lexical types.
...
{\sc {-}{-}ocompkey} values, as in the following lexical entry from
the ERG:
...
{\it abstain\_v1}: \[ \avmspan{\it v\_empty\_prep\_intrans\_le}\\
		     stem & \q< ``abstain'' \q>\\
                     synsem & \[ local.keys \[ key & \_abstain\_v\_from\_rel\\
                                               {-}{-}compkey & \_from\_p\_rel\_s \]\]\]
\end{avm}
}
...
associated with a noun will be found in the {\sc arg0} of their key
relation, although the index itself and the semantic agreement information
encoded on it should only be accessed through the {\sc hook.index}.
...
of the relation describing that event.  Again, this information and
the event index itself should never be accessed directly for 
semantic selection or composition.  The path {\sc hook.index} should
be used instead.
...
principles of semantic composition presented above.  The head-complement rule 
unifies the verb {\it try}'s constraints on
...
subject of {\it try}.  The constraints on {\it try}'s subject are propagated up
...
with the {\sc arg1} values in both the {\it try} relation and the {\it bark}
relation.
...
{\it \_seem\_v\_rel} which takes a proposition.
