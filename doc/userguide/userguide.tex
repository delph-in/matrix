...
The {\tt pos} component\mpt{what about a (adjective or adverb) and s (sahen)?} is one
...
omitted.  By convention, a predicate name with no sense component is
interpreted as underspecified for sense, so if more than one sense is
present in the lexicon for a given orthography and part of speech, each
of these predicate names should have a sense component.
...
processing (\namecite{Cop:03} also employs {\it leq} constraints.}
...
Matrix (v 0.6), the type {\it tam} (unsurprisingly) introduces the
...
{\it Kim abstained from the vote}:
...
\br{relation}{\br{\hspace{2.0in}arg0-relation}{\br{\hspace{-0.9in} noun-relation}{\lf{\hspace{-0.9in}named-relation}}
			     ...
			     \br{\hspace{1.0in} basic\_message}{}
...
modifier relations such as those expressed by (intersective) adverbs and by 
PPs modifying
verbal phrases.  Other elements that might appear to notionally take
events as arguments (e.g., clause-embedding verbs or scopal adverbs like
{\it probably}) actually take the handle of the phrase as their
argument.  The type {\it event-relation} is therefore constrained as follows:
...
the event index itself should be accessed via the path {\sc hook.index} 
...
would clearly be desirable if such an inventory could be defined.


\subsubsection{Messages}
\label{msgrelsec}

We provide the {\it message} type in the Matrix as an MRS encoding
of the distinctions among types of semantics of clauses developed in
\namecite{Gin:Sag:00}.  Clauses can express
commands, questions, or propositions, either as simple sentences or in
embedded contexts as sentential complements, relative clauses, etc.  The
type {\it message} is a subtype of {\it relation} which introduces the
attribute {\sc marg} (message argument), whose value will be the highest
scoping handle of the clause.  The {\sc pred} value of the message relation
is used to distinguish the three clause types, drawing from the pred sorts
given in the hierarchy in Figure~\ref{msgfig}\mpt{RMRS/Matrix
  mismatch: inconsistent in the text}, all of which carry the suffix
{\it \_m\_rel} for quick identification.
...
{\it
\begin{tree}
\br{message\_m\_rel}{\lf{command\_m\_rel}
             \br{\hspace{0.5in}prop-or-ques\_m\_rel}{
                               \br{abstr-ques\_m\_rel}{\lf{\hspace{-0.8in}question\_m\_rel}
                                              \lf{\hspace{0.5in}ne\_m\_rel}}
                               \lf{\hspace{1.0in}proposition\_m\_rel}}}
\end{tree}
}
...
{\it ne\_m\_rel}.\fn{On {\it ne\_rel}, see \citeboth{Ben:Fli:99}.}
...
entity as an argument.  This is achieved by giving {\it conjunction-relation}
...
%
The value of {\sc c-arg} is the conjoined index ({\it conj-index}) which
...
discussion of how constructions can contribute semantics.
...
the referential indices ({\it ref-ind}) must be bound by a quantifier.  In
...
value of an argument role in some relation {\sc arg1 ... arg4} is constrained
...
expected to appear as arguments of relations in an {\it mrs}.
...
{\sc mod} value.\mpt{maybe mention that these are less common than
