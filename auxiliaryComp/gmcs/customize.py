...
def specialize_word_order(hc,orders):
  ...
  if vcluster:
    ...
    mylang.add('basic-head-comp-phrase :+ [ SYNSEM.LOCAL.CAT.VC #vc, \
               ...
               section='addenda')
    if wo ==  'v2'
      aux-order = ch.get('aux-comp-order')
      if aux-order == 'before' or aux-order == 'both' or ch.get('vc-placement') == 'pre':
        initial-aux = True
      else:
        initial-aux = False 
      mylang.add('ĺex-or-phrase-synsem :+ [ SECOND luk ].', 'Introducing second keeps track whether verbs is in second position', section='addenda')
