...
def customize_matrix(path, arch_type, destination=None, force_dest=False):
    ...
    mylang.define_sections([['addenda', 'Matrix Type Addenda', True, False],
                            ['features', 'Language-specific Types for Feature Values', True, False],
                            ...
                            ['lvclex', 'Light Verb Constructions', False, False],
    ...
    lvc.init_light_verb_hierarchy(ch, hierarchies)
    ...
    # so other feature hierarchies must be saved later
    # TODO not sure if it matters whether this comes before/after
    #   customize_inflection or process_cfv_list
    lvc.customize_lvc(ch, mylang, rules)
    ...
    # Possible for disjunctive features to be generated by LVC library
    # so other feature hierarchies must be saved later
    # lvc.customize_lvc(mylang, ch, rules)
