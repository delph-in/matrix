  ...
  if lexical_supertype == 'verb':
    ...
    if choices.get('reflexives') == 'yes':
      ...
      lth.add_node(LexicalType('reflverb', get_lt_name('reflverb', choices),
                               parents={st:lth.nodes[st]}))
    if choices.get('verbal-particles') == 'yes':
      st = get_lexical_supertype('partverb', choices)
      lth.add_node(LexicalType('partverb', get_lt_name('partverb', choices),
                               parents={st:lth.nodes[st]}))
...
def get_lexical_supertype(lt_key, choices):
  ...
  if lexical_category in ('iverb','tverb','dverb','sc2verb','scontrverb','oraisverb, reflverb','partverb') and choices['has-aux'] == 'yes':
    return 'mverb'
...
def expand_lexical_supertype(st_key, choices):
  ...
  """
  i_t = ['iverb','tverb','dverb','sc2verb','scontrverb','oraisverb','cop','reflverb','partverb']
  m = ['mverb']
...
def get_lexical_supertypes(lrt_key, choices):
  ...
  if lexical_category == lrt_key:
    if lrt_key in ('iverb','tverb','dverb','sc2verb','scontrverb','oraisverb','reflverb','partverb'):
