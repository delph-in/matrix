...
####REFL
  ...
  if opt_refl:
    mylang.add('opt_refl-1st-comp-verb-lex := acc-1st-comp-verb-lex & \
         [ SYNSEM.LOCAL.CAT.VAL [ COMPS.FIRST.LOCAL refl-local & \
                                     [ CONT.HOOK.INDEX #ind ], \
                             SUBJ < [ LOCAL.CONT.HOOK.INDEX #ind ] > ] ].')
    climb_verbs.add('opt_refl-1st-comp-verb-lex := acc-1st-comp-verb-lex & \
         [ SYNSEM.LOCAL.CAT.VAL [ COMPS.FIRST.LOCAL refl-local & \
                                     [ CONT.HOOK.INDEX #ind ], \
                             SUBJ < [ LOCAL.CONT.HOOK.INDEX #ind ] > ] ].')
    comps_list.append('opt_refl')
####PART
  if part:
#particle is never followed by another argument
    ...
    if ch.get('circumpositions') == 'yes':
      mylang.add('+vp :+ [ PART-FORM list ].', section='addenda')
      climb_verbs.add('+vp :+ [ PART-FORM list ].', comment='section=\'addenda\'')
    else:
...
def create_basic_verb_types(ch, mylang, climb_verbs):
  ...
  pats = set(create_subcat_total_list(ch))
  if ch.get('subcat-regroup') == 'on':
    add_pats = set(create_subcat_with_opt_list(ch))
    for pat in add_pats:
      pats.add(pat)
...
def create_relevant_types(mylang, climb_verbs, sbj_cps, c, fa, cn):
  ...
  #  else:
    ...
    if '-comp' in name or '-sent' in name or '-wh' in name or '_comp' in name or '_sent' in name or '_wh' in name:
      ...
      if '-refl' in name or '_refl' in name:
