...
class ChoicesFile:
    ...
    def features(self):
        ...
        if 'q-infl' in self.choices and not 'wh-q-infl' in self.choices:
            ...
            features += [['question',
                          'polar|polar;wh|wh;both|polar and wh;no|no', '', 'verb', 'y']]
    ...
    def convert_32_to_33(self):
        if self.get('q-part-order') and not self.get('q-part'):
            self['q-part'] = 'on'
    ...
    def convert_34_to_35(self):
        ...
        for ns in self.get('ns'):
            #Only need to go through the remaining uprev code if this choices file comes from an older
            #grammar (one without a mandatory question regarding the valence of the nominalization strategy)
