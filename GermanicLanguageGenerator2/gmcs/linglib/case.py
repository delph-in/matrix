...
def case_names(ch):
  ...
  # ASF for Germanic: changing the case's name for those used in Germanic
  # languages
  cm = ch.get('case-marking')
  canon = []
  user = []
  if cm == 'nom-acc':
    canon.append('nominative')
    user.append(ch[cm + '-nom-case-name'])
    canon.append('accusative')
    user.append(ch[cm + '-acc-case-name'])
  elif cm == 'erg-abs':
    canon.append('ergative')
    user.append(ch[cm + '-erg-case-name'])
    canon.append('absolutive')
  ...
  # abbreviations
  # 
  # ASF Remove abbreviations (they become full form): if problems make
  # more elaborate changes to account for case
...
# For creating a case-hierarchy
def subcase_names(ch):
  subcases = []
  names = []
  superc = []
  for subcase in ch.get('subcase'):
...
# ASF no abbreviation in Germanic
...
def init_case_hierarchy(ch, hierarchies):
  ...
  if cm in ['nom-acc', 'erg-abs', 'tripartite', 'split-s', 'focus']:
    ...
    nom_a = canon_to_abbr('nominative', cases)
    acc_a = canon_to_abbr('accusative', cases)
