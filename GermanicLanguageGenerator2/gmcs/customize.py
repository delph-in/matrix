...
###ADD: check whether has auxiliaries (also consider revising general v2 impl)
...
###head-comp-phrase-2 and comp-head-phrase-2 are not needed for v2 with cluster
...
def specialize_word_order(hc,orders):
  ...
  if wo == 'v2' and ch.get('multiple-aux') == 'yes':
    vcluster = True
...
#ditransitive stuff is over
 ...
 #              section='addenda')check if necessary: guess also done elsewhere
    if wo == 'v2':
      auxorder = ch.get('aux-comp-order')
...
CONT.HOOK #hook ], \
  ...
  ###  add head-initial if necessary (test above)
...
def customize_verb_case():
  ...
  cases = case.case_names(ch)
  subcases = case.subcase_names(ch)
...
### Small adaptation to accomodate ditransitives
  patterns = ch.patterns()
  if ch.get('ditrans-case-marking') == 'nom-dat-acc':
    ditrans_case = ['nominative-dative-accusative', 'ditrans (nominative-dative-accuastive)', False]
    ditrans = ['ditrans', 'ditrans (case unspecified)', False]
    patterns.append(ditrans_case)
    patterns.append(ditrans)
###
  for p in patterns:
    rule_pattern = p[2]
    ...
    if not rule_pattern:
      ...
      if p[0] == 'ditrans' or len(c) > 2:  #ditransitive
        if p[0] == 'ditrans':
          a_case = ''
          i_case = ''
          o_case = ''
          ...
          i_case = case.canon_to_abbr(c[1], cases)
          o_case = case.canon_to_abbr(c[2], cases)
          a_head = ch.case_head(c[0])
          i_head = ch.case_head(c[1])
          o_head = ch.case_head(c[2])

        if a_case and i_case and o_case:
          t_type = dir_inv + a_case + '-' + i_case + '-' + o_case + '-ditransitive-verb-lex'
        else:
        ...
        # constrain the head of the indirect object
        typedef = \
          t_type + ' := \
          [ ARG-ST < [ ], [ LOCAL.CAT.HEAD ' + i_head + ' ], [ ] > ].'
        mylang.add(typedef)

        # constrain the case of the indirect object
        if i_case:
          typedef = \
            t_type + ' := \
            [ ARG-ST < [ ], [ LOCAL.CAT.HEAD.CASE ' + i_case + ' ] , [ ] > ].'
          mylang.add(typedef)

        # constrain CASE-MARKING of the patient/object, if appropriate
        if i_case and ch.has_mixed_case() and not ch.has_optadp_case(o_case):
          typedef = \
            t_type + ' := \
      ...
      elif p[0] == 'trans' or len(c) > 1:  # transitive
        if p[0] == 'trans':
          a_case = ''
          o_case = ''
          a_head = ch.case_head()
  ...
  for verb in ch.get('verb',[]):
    ...
    if val == 'trans':
      ...
      if len(c) > 2:  #ditrans
       ...
       i_case = case.canon_to_abbr(c[1], cases)
       o_case = case.canon_to_abbr(c[2], cases)
       tivity = a_case + '-' + i_case + '-' + o_case + '-ditrans'
      else:     #trans
       a_case = case.canon_to_abbr(c[0], cases)
       o_case = case.canon_to_abbr(c[1], cases)
...
#   2010-11-12 removing LIGHT + from comps to account for verbal clusters
# is added if not vc
        if vc:
          if ch.get('vc-analysis') == 'basic': 
            ...
            typedef = 'one-comp-aux := aux-lex & basic-one-arg & \
                 [ SYNSEM.LOCAL.CAT.VAL [ SUBJ < >, \
                                            COMPS < #comp >, \
                                            SPR < >, \
                                            SPEC < > ], \
...
# there can be more than on auxiliary per clause, this holds for all complements
        ...
        if sem == 'add-pred':
          ...
          mylang.add(typedef)
          if ch.get('vc-analysis') == 'basic':
            mylang.add(auxtypename + ' := [ ARG-ST < [ ], [ LOCAL.CONT.HOOK.LTOP #larg ] > ].')
          elif ch.get('vc-analysis') == 'aux-rule':
            mylang.add(auxtypename + ' := [ ARG-ST < [ LOCAL.CONT.HOOK.LTOP #larg ] > ].')
...
def customize_matrix(path, arch_type, destination=None):
  ...
  # TEMP SOLUTION, UNTIL POSTHEAD SOLUTION REVISED
  mypath = '/home/antske/delphin/matrix/branches/antske/acl2011-frozen'
  core_path = mypath + '/matrix-core'
